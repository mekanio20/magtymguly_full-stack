    ...
    <div class="relative">
      ...
      <div class="relative overflow-hidden">
        <div class="feather-background">
          <div class="sm:mt-16 mt-6 grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 grid-cols-1 sm:gap-x-12 gap-x-6 sm:gap-y-24 gap-y-14 relative z-1">
          <StatueCard v-for="statue in statues" :key="statue.id" :imgURL="`${$uploadUrl}/${statue.img}`" :name="statue.titleTm" :description="statue.locationTm" :icon="statue.icon"/>
        </div>
        </div>
      </div>
...
import StatueCard from "./StatueCard.vue";
export default {
  data() {
    return {
      underline: underline,
  ...
  methods: {
    async getStatues() {
      try {
        const statues = await this.$appAxios.get('/statues')
        this.statues = await statues.data.detail.rows
      } catch (error) {
        console.log(error)
      }
    }
